...
public class Player {
	...
	/**
	 * Counts the number of unmarked sticks in a given row.
	 * @param board the game board
	 * @param inRow the row to check
	 * @return the number of sticks in the given row.
	 */
	private int countSticksInRow(Board board, int inRow){
		int numOfSticks = 0;
		int maxSticks = board.getRowLength(inRow);
		for (int i = 1; i <= maxSticks; i++){
			if ( board.isStickUnmarked(inRow, i) ){
				numOfSticks ++;
			}
		}
		return numOfSticks;
	...
	private Move produceRandomMove(Board board){
		...
		int randomRow = 0,sticksInRow = 0, left = 0, sequence, success = 0;
		
		while (sticksInRow < 1){
			randomRow = myRandom.nextInt(board.getNumberOfRows()) + 1;
			sticksInRow = countSticksInRow(board,randomRow);	
		}
		
		while (success < 1){
			if (success == 0)
				left = myRandom.nextInt(sticksInRow) + 1;
			sequence = myRandom.nextInt(sticksInRow - left) + 1;
			myMove = new Move(randomRow, left, left + sequence);
	...
	private Move produceHumanMove(Board board){
		Move myMove;
		int userInput = 0, inRow, inLeft, inRight;
		...
		while (true){
			...
			case 2:
				...
				inLeft = scanner.nextInt();
				System.out.println("Enter the index of the rightmost stick:");
				inRight = scanner.nextInt();
				myMove = new Move(inRow, inLeft, inRight);
				int success = board.markStickSequence(myMove);
				if (success > 0){
					return myMove;
				}
				break;
